...
class MemberFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    {
        $alumnusStatus = $manager->getRepository(MemberStatus::class)->findOneBy(['code' => 'ALUMNUS']);
        $tag1901Club = $this->getReference(TagFixtures::TAG_1901_CLUB);

        if (is_null($alumnusStatus)) {
            throw new \Exception('Unable to load MemberStatus, skipping loading members.');
            return;
        }
        ...
        $member->setStatus($alumnusStatus);
